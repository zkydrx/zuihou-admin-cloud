...
public class PoiTest
{
    ...
    static
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            USER_LIST.add(user);
            UserExcelVO userEX = UserExcelVO.builder().account("账号" + i).name("名字" + i).mobile("15218869970").sex(i % 2 == 0 ? Sex.M : Sex.M).nation("汉族" + i)
                                            //                    .nation(new RemoteData<>("HELLO" + i, "汉族" + i))
                                            .workDescribe("工作描述工作描述工作描述工作描述工作描述工作描述工作描述." + i).status(true).lastLoginTime(LocalDateTime.now()).build();
            if (i == 0)
            {
                userEX.setSex(null);
            }
    ...
    public void testImport() throws Exception
    {
        ...
        ExcelImportResult<Station> result = ExcelImportUtil.importExcelMore(new File(OUTPUT_PATH + "导出岗位数据.xlsx"),
    ...
    public void testImportUser() throws Exception
    {
        ...
        ExcelImportResult<UserExcelVO> result = ExcelImportUtil.importExcelMore(new File(OUTPUT_PATH + "User.xlsx"), UserExcelVO.class, params);
